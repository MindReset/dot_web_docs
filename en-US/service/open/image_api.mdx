...
<Callout type="info" title="About image input">
	The `image` field supports either PNG Base64 data or a full online `http(s)` image URL. Remote image URLs must be anonymously accessible, require no authentication or special Referer, and return `image/*` content directly.
</Callout>
...
<ApiPlayground
	...
	bodyParams={{
		...
		image: {
			type: 'textarea',
			required: true,
			name: 'PNG Base64 image data or http(s) image URL',
		},
		link: {
			type: 'string',
...
<Tabs items={['cURL']}>
  <Tab value="cURL">
    ...
    curl -X POST \
      ...
      -d '{
        "refreshNow": false,
        "image": "{{base64-encoded PNG image data or http(s) image URL}}",
...
<TypeTable
	type={{
		...
		image: {
			description:
				'Base64-encoded PNG image data or full http(s) image URL (296px×152px, and remote URLs must be anonymously accessible and return image/* directly) (Image to render on screen)',
			type: 'string',
			required: true,
		},
		link: {
