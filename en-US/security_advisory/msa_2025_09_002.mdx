...
**Severity**: Critical  
**Status**: In progress (temporary mitigations in place; permanent fix expedited)  
...
| **Affected Component** | /api/device/template (MongoDB aggregation pipeline execution)                                        |
...
-               Reproduction Notes: By crafting an aggregation containing high-risk stages such as $unionWith, it is possible to access data from other collections without authorization.
-               Risk: Confidentiality (C) High, Integrity (I) High, Availability (A) High.
...
-               Immediate Actions (in progress):
    -   Temporarily disable or gradually restrict high-risk operators in /api/device/template, enforce strict field whitelisting and rate limiting, and block suspicious aggregation stages (e.g., $unionWith/$lookup/$out/$merge).
    -   Increase request auditing and alerting to identify suspicious sources and access frequency.
-               Permanent Fix (to be completed and released ASAP):
...
-               If you observe abnormal responses or suspicious behavior, please report via official channels promptly.
