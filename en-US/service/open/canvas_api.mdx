...
<Callout type="info" title="How screen structure works">
	Canvas API uses `windowData` to describe the screen with object elements: each element is an object, `type` selects the element type, `props` contains style and content, and `props.children` can be text, one child element, or an array of child elements.
</Callout>
	...
	bodyParams={{
		...
		data: {
			...
			example: JSON.stringify(
				{
					title: 'Canvas API',
					message: 'Build a custom device screen from live data.',
					badge: 'OPENAPI',
				},
				null,
				2
