...
<ApiPlayground
	...
	pathParams={{
		...
		taskType: {
			...
			name: '任务类型',
			default: 'loop',
			options: [{ value: 'loop', label: '循环任务' }],
			description: '当前仅开放循环任务的查询。',
		},
	}}
	auth={true}
	defaultOpen={false}
...
- `deviceId`: 设备序列号
- `taskType`: 任务类型，当前仅开放 `loop`（循环任务）查询

### 响应结构

返回值为任务数组。

- `type`: 任务内容类型（仅返回 `TEXT_API` / `IMAGE_API`）
- `key`: 任务唯一标识（可能为 `null`）
- 其余字段：该任务的输入参数（不同类型的任务字段不同，例如 Text API 的 `title` / `message`，Image API 的 `border` / `ditherType` 等）
	...
	type={{
		'[]': {
			...
			items: {
				...
				key: {
					description: '任务唯一标识（用于 Text/Image API 的 taskKey 参数）',
					type: 'string | null',
					required: true,
				},
				refreshNow: {
