...
<Callout type="warning" title="旧エンドポイントの状態">
	Status: Deprecated. 旧エンドポイント `https://dot.mindreset.tech/api/open/image` へのリクエストは、現在は新しい API へ自動転送されます。
    ...
    - 本ドキュメントのリクエスト構造に従ってパラメータを送信（例：`refreshNow` / `taskAlias` / `image` / `link` / `border` / `ditherType` / `ditherKernel` / `taskKey`）
...
5. Dot. アプリの創造所から「画像 API」コンテンツをデバイスの **ループ** タスクに追加していること

<Callout type="info" title="Image API の用途">
	Image API は、ポスター、スクリーンショット、ダッシュボード、お気に入りの画像をデバイスに送ります。すでに完成したビジュアルを、そのまま表示したい場合に向いています。
</Callout>
...
<Callout type="info" title="画像入力について">
	`image` フィールドは必須です。PNG Base64 データまたは完全なオンライン `http(s)` 画像 URL を指定する必要があります。PNG Base64 は裸の Base64、または `data:image/png;base64,` プレフィックス付きの形式を使用でき、デコード後のサイズは最大 3MB です。リモート画像 URL は匿名で直接アクセスでき、認証や特別な Referer を必要とせず、URL 長は 2048 文字以内、レスポンス本文は最大 3MB で、`image/*` を直接返す必要があります。
</Callout>

<Callout type="info" title="デバッグ用のテスト画像 URL">
	以下の URL は現時点で匿名アクセス可能で、Image API で利用できる画像コンテンツを返します。

	- `https://placehold.co/600x400/png`
	- `https://httpbin.org/image/png`

	自分の画像 URL が使えるか先に確認したい場合は、次を実行してください。
...
<ApiPlayground
	...
	bodyParams={{
		...
		taskKey: {
			...
			name: 'タスク識別子',
			description:
				'「デバイスタスク一覧」API で取得できます（複数の「画像 API」コンテンツがある場合に対象を指定）。',
		},
		taskAlias: {
			type: 'string',
			name: 'タスク別名',
			description:
				'デバイスタスク一覧で複数の Image API コンテンツを区別するために使います。表示画像には影響しません。',
		},
		image: {
			type: 'textarea',
			required: true,
			name: 'PNG Base64 画像データ（最大 3MB）または http(s) 画像 URL',
		},
		link: {
			type: 'string',
			name: 'タッチ操作時の遷移リンク',
...
```json
{
	"message": "デバイス ABCD1234ABCD 画像 API コンテンツが切り替わりました。"
}
```
...
<Tabs items={['cURL']}>
  <Tab value="cURL">
    ...
    curl -X POST \
      ...
      -d '{
        "refreshNow": false,
        "image": "{{PNG Base64 画像データ（最大 3MB）または http(s) 画像 URL}}",
...
<TypeTable
	type={{
		...
		image: {
			description:
				'必須。PNG Base64 画像データ（裸の Base64 または data:image/png;base64, プレフィックス付き、デコード後最大 3MB）または完全な http(s) 画像 URL（296px×152px、かつリモート URL は匿名アクセス可能、2048 文字以内、レスポンス本文最大 3MB、image/* を直接返す必要があります）（表示する画像）',
			type: 'string',
			required: true,
		},
		link: {
		...
		taskKey: {
			description:
				'デバイスに複数の「画像 API」コンテンツがある場合、更新して切り替える対象を taskKey で指定します。taskKey は「デバイスタスク一覧」で取得できます。未指定の場合は最初の画像 API コンテンツが選択されます。',
			type: 'string',
		},
		taskAlias: {
			description:
...
| `404`           | 未検出           | デバイスが存在しない・未登録／画像 API コンテンツがループタスクに未追加                                           |
