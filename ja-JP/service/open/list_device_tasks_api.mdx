---
title: デバイスタスク一覧
description: API を通じてデバイスの現在のタスクリストを取得します。
---
...
<ApiPlayground
	...
	pathParams={{
		...
		taskType: {
			...
			default: 'loop',
			options: [
				{ value: 'loop', label: 'ループコンテンツ' },
				{ value: 'fixed', label: '固定コンテンツ' },
			],
		},
	}}
	auth={true}
...
- `taskType`: コンテンツ種別（例: `loop` ループコンテンツ）

### レスポンス構造

レスポンスはコンテンツ配列です。

- `type`: コンテンツ種別（`TEXT_API` / `IMAGE_API` / `CANVAS_API` / `GENERAL`）
- `key`: コンテンツ識別子（`null` の場合あり）
- その他のフィールド：コンテンツの入力データ（コンテンツ種別により異なる）
...
<TypeTable
	type={{
		'[]': {
			...
			items: {
				type: {
					description: 'コンテンツ種別（`TEXT_API` / `IMAGE_API` / `CANVAS_API` / `GENERAL`）',
					type: "'TEXT_API' | 'IMAGE_API' | 'CANVAS_API' | 'GENERAL'",
					required: true,
				},
				key: {
					description: 'コンテンツ識別子（Text/Image/Canvas API の taskKey として使用）',
					type: 'string | null',
					required: true,
				},
				refreshNow: {
