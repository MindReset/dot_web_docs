---
title: Get Device Status
---

import Link from 'fumadocs-core/link';
import { ApiPlayground } from 'src/shadcn/components/api-playground';
import { TypeTable } from 'src/shadcn/components/type-table';

## Quick Guide

### Prerequisites

1. Make sure you have <Link href="/docs/service/studio/api/get_api">retrieved and saved your API key</Link> in the Dot. App;
2. Make sure you own at least one device and have <Link href="/docs/service/studio/api/get_device_id">obtained its device serial number</Link>;

<Callout
	type="info"
	title="The API rate limit is 10 requests per second. With the API, you can control device behavior and responses more flexibly."
/>

<Callout
	type="info"
	title="Placeholders wrapped in '{{variable}}' indicate values you must replace. Fill them with your actual values and remove the '{{}}' braces."
/>

### Request Examples

<ApiPlayground
	baseUrl="https://dot.mindreset.tech"
	endpoint="/api/authV2/open/device/:id/status"
	method="GET"
	pathParams={{
		id: {
			type: 'string',
			required: true,
			name: 'Device serial number',
		},
	}}
	auth={true}
	defaultOpen={false}
/>

### Response Example

```json
{
	"deviceId": "ABCD1234ABCD",
	"alias": null,
	"location": null,
	"status": {
		"version": "1.0.0",
		"current": "Power Active",
		"description": "The device is power active and ready to use",
		"battery": "Charging",
		"wifi": "-62 dBm"
	},
	"renderInfo": {
		"last": "12/18/2025 14:11",
		"current": {
			"rotated": false,
			"border": 0,
			"image": ["https://example.com/render/0.png", "https://example.com/render/1.png"]
		},
		"next": {
			"battery": "12/18/2025 17:11",
			"power": "12/18/2025 14:16"
		}
	}
}
```

## Troubleshooting

### Endpoint

`/api/authV2/open/device/:id/status`

### Method

`GET`

### Path Parameters

<TypeTable
	type={{
		id: {
			description: 'Device serial number',
			type: 'string',
			required: true,
		},
	}}
/>

### Response Schema

<TypeTable
	type={{
		deviceId: {
			description: 'Device serial number',
			type: 'string',
			required: true,
		},
		alias: {
			description: 'Device alias',
			type: 'string | null',
		},
		location: {
			description: 'Device location',
			type: 'string | null',
		},
		status: {
			description: 'Device status information',
			type: 'object',
			required: true,
			parameters: {
				version: {
					description: 'Software version',
					type: 'string',
					required: true,
				},
				current: {
					description: 'Current device status',
					type: 'string',
					required: true,
				},
				description: {
					description: 'Detailed description of device status',
					type: 'string',
					required: true,
				},
				battery: {
					description: 'Battery status',
					type: 'string',
					required: true,
				},
				wifi: {
					description: 'WiFi signal strength',
					type: 'string',
					required: true,
				},
			},
		},
		renderInfo: {
			description: 'Render timing information',
			type: 'object',
			required: true,
			parameters: {
				last: {
					description: 'Time when device last rendered',
					type: 'string',
					required: true,
				},
				current: {
					description: 'Current render information',
					type: 'object',
					required: true,
					parameters: {
						rotated: {
							description: 'Whether the screen is rotated',
							type: 'boolean',
							required: true,
						},
						border: {
							description: 'Border color (0 = white, 1 = black)',
							type: 'number',
							required: true,
						},
						image: {
							description: 'Current rendered image URLs',
							type: 'string[] | null',
						},
					},
				},
				next: {
					description: 'Next scheduled render times',
					type: 'object',
					required: true,
					parameters: {
						battery: {
							description: 'Next rendering time in battery mode',
							type: 'string',
							required: true,
						},
						power: {
							description: 'Next rendering time in power mode',
							type: 'string',
							required: true,
						},
					},
				},
			},
		},
	}}
/>

### Status Codes

| code  | Meaning        | Description                          |
| :---- | :------------- | :----------------------------------- |
| `200` | Success        | Successfully retrieved device status |
| `400` | Invalid params | Invalid device ID format             |
| `401` | Unauthorized   | Invalid or expired API key           |
| `403` | Forbidden      | No permission to view this device    |
| `404` | Not found      | Device not found or not registered   |
| `500` | Server Error   | Internal server error                |
