...
- `type`: 内容类型（返回 `TEXT_API` / `IMAGE_API` / `CANVAS_API` / `GENERAL`）
- `key`: 内容唯一标识（可能为 `null`）
...
<TypeTable
	type={{
		'[]': {
			...
			items: {
				type: {
					description: '内容类型（`TEXT_API` / `IMAGE_API` / `CANVAS_API` / `GENERAL`）',
					type: "'TEXT_API' | 'IMAGE_API' | 'CANVAS_API' | 'GENERAL'",
					required: true,
				},
				key: {
					description: '内容唯一标识（可用于 Text/Image/Canvas API 的 taskKey 参数）',
					type: 'string | null',
					required: true,
				},
				refreshNow: {
