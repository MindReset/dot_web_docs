...
<Callout type="info" title="关于图像输入">
	`image` 字段支持传递 PNG Base64 数据，也支持传递完整的在线 `http(s)` 图片 URL。远程图片链接必须可匿名访问、无需鉴权或特殊 Referer，并直接返回 `image/*` 内容。
</Callout>
...
<ApiPlayground
	...
	bodyParams={{
		...
		image: {
			type: 'textarea',
			required: true,
			name: 'PNG Base64 图像数据或 http(s) 图片 URL',
		},
		link: {
			type: 'string',
			name: '碰一碰跳转链接',
...
<Tabs items={['cURL']}>
  <Tab value="cURL">
    ...
    curl -X POST \
      ...
      -d '{
        "refreshNow": false,
        "image": "{{PNG Base64 图像数据或 http(s) 图片 URL}}",
...
<TypeTable
	type={{
		...
		image: {
			description:
				'PNG Base64 图像数据或完整 http(s) 图片 URL（296px×152px，且远程链接需可匿名访问并直接返回 image/*）（屏幕呈现的图像）',
			type: 'string',
			required: true,
		},
		link: {
