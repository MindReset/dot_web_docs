...
<Callout type="info" title="画像入力について">
	`image` フィールドには、PNG Base64 データまたは完全なオンライン `http(s)` 画像 URL を指定できます。リモート画像 URL は匿名で直接アクセスでき、認証や特別な Referer を必要とせず、`image/*` を返す必要があります。
</Callout>
...
<ApiPlayground
	...
	bodyParams={{
		...
		image: {
			type: 'textarea',
			required: true,
			name: 'PNG Base64 画像データまたは http(s) 画像 URL',
		},
		link: {
			type: 'string',
			name: 'タッチ操作時の遷移リンク',
...
<Tabs items={['cURL']}>
  <Tab value="cURL">
    ...
    curl -X POST \
      ...
      -d '{
        "refreshNow": false,
        "image": "{{PNG Base64 画像データまたは http(s) 画像 URL}}",
...
<TypeTable
	type={{
		...
		image: {
			description:
				'PNG Base64 画像データまたは完全な http(s) 画像 URL（296px×152px、かつリモート URL は匿名アクセスで image/* を直接返す必要があります）（表示する画像）',
			type: 'string',
			required: true,
		},
		link: {
